...
%HW1-Problem2
%Use the Average method to predict SG+SR(No wind/ No slope condition)
%Result SG+SR = 2.3068e+03
...
%Coefficient
%Cl_opt = 2*W/(Rho_Slugsft3 * Sref_Ft2 * (V_G.^2));
%Wrong, this is actually Cl_max

Cl_opt = mu/(2*k);
%Cl = 2*W/(Rho_Slugsft3*Sref_Ft2*(V).^2);
C_d = Cd_o + k * Cl_opt.^2;
...
q_avg = 1/2*(Rho_Slugsft3*(V_avg).^2);
